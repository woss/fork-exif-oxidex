...
///
/// This converts the Vec<(u16, Vec<u8>)> format returned by parse_tiff_file
/// into a MetadataMap with proper TagValue objects.
fn tags_to_metadata_map(tags: Vec<(u16, Vec<u8>)>) -> MetadataMap {
...
fn test_round_trip_tiff_modification() {
    ...
    let make_tag = new_tags.iter().find(|(id, _)| *id == 0x010F);
    assert!(make_tag.is_some(), "Make tag should be present in output");

    let (_, make_value) = make_tag.unwrap();
    ...
    let iso_tag = new_tags.iter().find(|(id, _)| *id == 0x8827);
    assert!(iso_tag.is_some(), "ISO tag should be present in output");

    if let Some((_, iso_value)) = iso_tag {
        if iso_value.len() >= 2 {
...
fn test_write_tiff_with_multiple_tag_types() {
    ...
    // Verify ISO (integer)
    let iso = tags.iter().find(|(id, _)| *id == 0x8827);
    assert!(iso.is_some(), "ISO tag should be present");

    // Verify FNumber (rational)
    let fnumber = tags.iter().find(|(id, _)| *id == 0x829D);
