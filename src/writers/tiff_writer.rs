...
mod tests {
    ...
    fn test_round_trip_little_endian() {
        ...
        // Verify Make
        let make = parsed.iter().find(|(id, _)| *id == 0x010F);
        assert!(make.is_some());
        let (_, make_value) = make.unwrap();
        assert_eq!(make_value, b"Canon\0");

        // Verify Model
        let model = parsed.iter().find(|(id, _)| *id == 0x0110);
        assert!(model.is_some());
        let (_, model_value) = model.unwrap();
        assert_eq!(model_value, b"EOS\0");

        // Verify ISO
        let iso = parsed.iter().find(|(id, _)| *id == 0x8827);
        assert!(iso.is_some());
        let (_, iso_value) = iso.unwrap();
    ...
    fn test_round_trip_with_rational() {
        ...
        let fnumber = parsed.iter().find(|(id, _)| *id == 0x829D);
        assert!(fnumber.is_some());
        let (_, value) = fnumber.unwrap();
