---
title: JPEG Compatibility
---

# JPEG Compatibility Report

**Generated:** 2025-12-08 | **ExifTool:** v13.43 | **OxiDex:** v1.2.1

## Summary

- **Files Tested:** 4085
- **Coverage:** 14.5%
- **Matched Tags:** 533
- **Missing Tags:** 2879
- **Extra Tags:** 415
- **Value Differences:** 262

## Value Differences

Tags where ExifTool and OxiDex extract different values:

| Tag | ExifTool | OxiDex |
|-----|----------|--------|
| `APP11:Alpha` | 1.0 | 71496341941566142676992 |
| `APP11:Beta` | 1.0 | 0 |
| `APP11:CorrectionMethod` | 0 | 118 |
| `APP11:JPEG-HDRVersion` | 11 | 32 |
| `APP11:Ln0` | 0.122262 | 13635833219286100554547200000 |
| `APP11:Ln1` | 2.634655 | 0 |
| `APP11:RatioImage` | (Binary data 19 bytes, use -b option to ... | (Binary data 85 bytes, use -b option to ... |
| `APP11:S2n` | 2269.635 | 0 |
| `Ducky:Copyright` | Copyright 2004 Phil Harvey | Copyright 2004 Phil Harvey |
| `EXIF:Acceleration` | 0 | -0 |
| `EXIF:AmbientTemperature` | -0 C | -0 |
| `EXIF:CFAPattern` | [Blue,Green][Green,Red] | [Invalid CFA data] |
| `EXIF:CompositeImageExposureTimes` | 0 3.4 0 0.4 0 0.4 0 0 | (Binary data 58 bytes, use -b option to ... |
| `EXIF:Compression` | JPEG (old-style) | Uncompressed |
| `EXIF:ExposureIndex` | 3.614421976e-10 | -0.000000001 |
| `EXIF:GPSAreaInformation` |  | (Binary data 264 bytes, use -b option to... |
| `EXIF:GPSDestBearing` | 358.8270572 | 358.827057183 |
| `EXIF:GPSDestDistanceRef` | Unknown () |  |
| `EXIF:GPSImgDirection` | 358.8270572 | 358.827057183 |
| `EXIF:GPSLatitude` | 51 deg 29' 42.41" | 51 deg 29' 0" |
| `EXIF:GPSLongitude` | 3 deg 13' 5.95" | 3 deg 13' 0" |
| `EXIF:GPSSpeed` | 0.2700000107 | 0.270000011 |
| `EXIF:ImageHeight` | 60 | 960 |
| `EXIF:ImageWidth` | 80 | 720 |
| `EXIF:LensInfo` | 1.570000052-9mm f/1.5-2.8 | 1.57-9mm f/1.5-2.8 |
| `EXIF:PanasonicTitle2` | 9999:99:99 00:00:00 | (Binary data 128 bytes, use -b option to... |
| `EXIF:Pressure` | 0 | -0 |
| `EXIF:UserComment` |  | 425412A490439B448438t470428T0V14.2FR999.... |
| `EXIF:WaterDepth` | -0.0 | -0 |
| `EXIF:XPKeywords` | Cali | Cali |
| `EXIF:XPTitle` |  |                 |
| `ICC_Profile:DeviceModel` |  |  |
| `IPTC:Country-PrimaryLocationCode` |  | HRV |
| `IPTC:SupplementalCategories` | ["amazing","image","utilities"] | utilities |
| `IPTC:Urgency` | 8 (least urgent) | 8 |
| `MPF:MPImageStart` | 3406314 | 3387627 |
| `MakerNotes:AELock` | On | Off |
| `MakerNotes:AESetting` | Normal AE | Unknown (-1) |
| `MakerNotes:AFAreaMode` | Single-point AF | 1-Area (high speed) |
| `MakerNotes:AFIlluminator` | Auto | 1 |
| `MakerNotes:AFImageHeight` | 2592 | 45 |
| `MakerNotes:AFImageWidth` | 3888 | 45 |
| `MakerNotes:AFPoint` | 0 | Unknown (0) |
| `MakerNotes:AFPointInFocus` | Left (vertical) | 5 |
| `MakerNotes:AFPointPosition` | n/a | 10228 |
| `MakerNotes:AFPointSelected` | Face Detect AF | 65535 |
| `MakerNotes:AFPointsInFocus` | Center | 112 |
| `MakerNotes:AFPointsSelected` | 13 | 112 |
| `MakerNotes:AccelerometerX` | 5 | 6547.7° |
| `MakerNotes:AccelerometerY` | 22 | 6535.8° |

*...and 212 more differences*

## Missing Tags

Tags ExifTool extracts that OxiDex doesn't:

| Tag | Sample Value |
|-----|-------------|
| `APP0:InterleavedField` | Not Interleaved |
| `APP0:OcadRevision` | 14797 |
| `APP10:HDRGainCurve` | (Binary data 1467 bytes, use -b option to extract) |
| `APP10:HDRGainCurveSize` | 188 |
| `APP12:CAM1` | 33 |
| `APP12:CAM2` | 33 |
| `APP12:CAM3` | 0 |
| `APP12:CAM4` | 0 |
| `APP12:CAM5` | 0 |
| `APP12:CAM6` | 0 |
| `APP12:CAM7` | 0 |
| `APP12:CAM8` | 0 |
| `APP12:CAM9` | 0 |
| `APP12:COLOR1` | 21083278 |
| `APP12:COLOR2` | 18132941 |
| `APP12:COLOR3` | 17387092 |
| `APP12:COLOR4` | 0 |
| `APP12:CameraType` | SR84 |
| `APP12:ColorMode` | 1 |
| `APP12:ContTake` | 0 |
| `APP12:DateTimeOriginal` | 1998:12:31 15:17:20 |
| `APP12:EXP1` | 6052 |
| `APP12:EXP2` | 33 |
| `APP12:EXP3` | 237 |
| `APP12:ExposureCompensation` | +2.0 |
| `APP12:ExposureTime` | 1/155 |
| `APP12:FCS1` | 2853 |
| `APP12:FCS2` | 2699 |
| `APP12:FCS3` | 2849 |
| `APP12:FCS4` | 218 |
| `APP12:FCS5` | 214 |
| `APP12:FCS6` | 3 |
| `APP12:FCS7` | 3 |
| `APP12:FNumber` | 11.0 |
| `APP12:Flash` | Off |
| `APP12:Fnumber` | F2.8 |
| `APP12:ID` | AGFA DIGITAL CAMERA |
| `APP12:IMbb` | 35761 |
| `APP12:IMbg` | 33709 |
| `APP12:IMbr` | 32929 |
| `APP12:IMgb` | 33346 |
| `APP12:IMgg` | 35931 |
| `APP12:IMgr` | 33122 |
| `APP12:IMrb` | 32721 |
| `APP12:IMrg` | 33975 |
| `APP12:IMrr` | 35704 |
| `APP12:ImageSize` | 1280x960 |
| `APP12:JPEG1` | 130432 |
| `APP12:LightS` | 0 |
| `APP12:MODE1` | 0 |
| `APP12:MODE2` | 0 |
| `APP12:MODE3` | 0 |
| `APP12:MODE4` | 0 |
| `APP12:MODE5` | 1 |
| `APP12:MODE6` | 1 |
| `APP12:MTR1` | 257 |
| `APP12:MTR2` | 214 |
| `APP12:MTRX1` | 1 |
| `APP12:Macro` | Off |
| `APP12:PicLen` | 561039 |
| `APP12:Protect` | 0 |
| `APP12:REV` | DCPT |
| `APP12:Resolution` | 5 |
| `APP12:S0` | 1e8,0,11b0,6f72,15cf,4225,4225,1050000,a1e0004,0,2... |
| `APP12:STB1` | 139 |
| `APP12:STB2` | 0 |
| `APP12:STB3` | 262 |
| `APP12:STB4` | 14 |
| `APP12:STB5` | 0 |
| `APP12:STB6` | 0 |
| `APP12:SerialNumber` | #00000001 |
| `APP12:T0` | 11b15600,1290000,e00c0f,2,0,0 |
| `APP12:TagB` | 332 |
| `APP12:TagQ` | 96 |
| `APP12:TagR` | 293 |
| `APP12:TagS` | v |
| `APP12:ThmLen` | 3802 |
| `APP12:Version` | v84-71 |
| `APP12:WB1` | 2 |
| `APP12:WB2` | 5528 |
| `APP12:WB3` | 285,7 |
| `APP12:WB4` | 216,6 |
| `APP12:WB5` | 0 |
| `APP12:WB6` | 0 |
| `APP12:Zoom` | 2.1 |
| `APP13:AdobeCMType` | 3 |
| `APP15:Quality` | 70 |
| `APP1:AboveColor` | 170 128 128 |
| `APP1:AtmosphericTemperature` | 20.0 C |
| `APP1:AtmosphericTransAlpha1` | 0.006569 |
| `APP1:AtmosphericTransAlpha2` | 0.01262 |
| `APP1:AtmosphericTransBeta1` | -0.002276 |
| `APP1:AtmosphericTransBeta2` | -0.00667 |
| `APP1:AtmosphericTransX` | 1.9 |
| `APP1:BelowColor` | 50 128 128 |
| `APP1:CameraModel` | FLIR_i7 |
| `APP1:CameraPartNumber` | T197600 |
| `APP1:CameraSerialNumber` | 470023842 |
| `APP1:CameraSoftware` | 8.1.1 |
| `APP1:CameraTemperatureMaxClip` | 270.0 C |

*...and 2779 more missing tags*

## Extra Tags

Tags OxiDex extracts that ExifTool doesn't:

| Tag | Value |
|-----|-------|
| `Casio:CCDSensitivity` | 64 |
| `Casio:Contrast` | 0 |
| `Casio:DigitalZoom` | 1 |
| `Casio:FlashIntensity` | 13 |
| `Casio:FlashMode` | 4 |
| `Casio:FocusMode` | 3 |
| `Casio:Quality` | 3 |
| `Casio:RecordingMode` | 1 |
| `Casio:Saturation` | 0 |
| `Casio:Sharpness` | 0 |
| `Casio:WhiteBalance` | 1 |
| `Composite:Aperture` | 7.1 |
| `Composite:ISO` | 64 |
| `Composite:ImageSize` | 720x960 |
| `Composite:Megapixels` | 0.691 |
| `Composite:ShutterSpeed` | 1/250 |
| `DJI:Make` | DJI |
| `EXIF:0x0000` | 1.0.0.0 |
| `EXIF:0x004B` | 1 |
| `EXIF:0x0103` | 6 |
| `EXIF:0x012D` | 0 0 2 4 7 11 17 24 32 42 53 65 79 94 111 129 148 1... |
| `EXIF:0x0201` | 568 |
| `EXIF:0x0202` | 36713 |
| `EXIF:0x0220` | 0 |
| `EXIF:0x0221` | 0 |
| `EXIF:0x0222` | 0 |
| `EXIF:0x0223` | 0 |
| `EXIF:0x0224` | 0 |
| `EXIF:0x0225` |  |
| `EXIF:0x827D` | K520C-01044 |
| `EXIF:0x83BB` | (Binary data 4255 bytes, use -b option to extract) |
| `EXIF:0x882A` | 2 |
| `EXIF:0x9103` | 1 |
| `EXIF:0x920C` | 474561179 |
| `EXIF:0x920D` | 10 |
| `EXIF:0x920E` | 3744 |
| `EXIF:0x920F` | 3744 |
| `EXIF:0x9210` | 2 |
| `EXIF:0x927C` | (Binary data 1570 bytes, use -b option to extract) |
| `EXIF:0x9284` | 160 |
| `EXIF:0x9285` | 103 |
| `EXIF:0x9287` | 4294967186 |
| `EXIF:0x9288` | 4294967195 |
| `EXIF:0x9289` | 4294967189 |
| `EXIF:0x9CA1` | (Binary data 60 bytes, use -b option to extract) |
| `EXIF:0x9CA2` | MBAAAEAFCAAAAAAAADAAAAAAAYUgAAAAAAAAAAAAAAAAAAAAAA... |
| `EXIF:0xC4A5` | (Binary data 28 bytes, use -b option to extract) |
| `EXIF:0xDD01` |  |
| `EXIF:0xDD02` |  |
| `EXIF:0xDD03` |  |

*...and 365 more extra tags*

---

[← Back to Overview](./)
