...
fn generate_rust_code(tags: &[TagDefinition]) -> Result<()> {
    ...
    for domain in &domains {
        ...
        let output_path = format!("oxidex-tags-{}/src/{}_tags.yaml", domain, domain);
...
/// Generates a compatibility facade for legacy generated_tags.rs
///
/// This provides backward compatibility during the migration to YAML-based tags.
/// The facade maintains the same API while delegating to the active registry.
fn generate_legacy_rust_stub(tags: &[TagDefinition]) -> Result<()> {
    ...
    // Write compatibility facade that maintains API compatibility.
    writeln!(
        file,
    ...
    writeln!(file, "//! `oxidex-tags-*` domain crates.")?;
    writeln!(file, "//!")?;
    writeln!(file, "//! Generated from ExifTool: {}", EXIFTOOL_REPO_URL)?;
    writeln!(file, "//! Parsed tag definitions: {}", tags.len())?;
    writeln!(file)?;
