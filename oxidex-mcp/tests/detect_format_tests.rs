...
async fn test_detect_format_single_jpeg() {
    ...
    assert!(
        output.contains("Format: JPEG"),
        "Output should identify JPEG format"
    );
    assert!(
    ...
    assert!(
        output.contains("Supported Metadata:"),
        "Output should list metadata groups"
    );
...
async fn test_detect_format_glob_pattern() {
    ...
    assert!(
        result.is_ok(),
        "detect_format should succeed with glob: {:?}",
        result
    );
...
async fn test_detect_format_nonexistent_file() {
    ...
    assert!(
        result.is_ok(),
        "detect_format should handle missing file gracefully"
    );

    let output = result.unwrap();
    assert!(
        output.contains("File not found"),
        "Output should indicate file not found"
    );
}
...
async fn test_detect_format_empty_glob() {
    ...
    assert!(
        result.is_ok(),
        "detect_format should handle empty glob gracefully"
    );

    let output = result.unwrap();
    assert!(
        output.contains("No files matched pattern"),
...
async fn test_detect_format_directory_traversal_blocked() {
    ...
    assert!(
        result.is_err(),
        "detect_format should reject directory traversal"
    );

    let error = result.unwrap_err();
    assert!(
        error.to_string().contains("directory traversal"),
