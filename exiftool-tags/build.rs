...
fn generate_rust_code(tags: &[TagDefinition]) -> Result<()> {
    ...
    writeln!(file, "//! Total tags: {} across {} format families", tags.len(), family_modules.len())?;
    writeln!(file)?;
    writeln!(file, "#![allow(dead_code)]")?;
    writeln!(file)?;
    writeln!(file, "use crate::tag_db::TagDescriptor;")?;
    ...
    writeln!(file, "pub fn get_generated_tag_descriptor(name: &str) -> Option<&'static TagDescriptor> {{")?;
    for (module_name, _) in &family_modules {
        writeln!(file, "    if let Some(desc) = {}::get_tags().get(name) {{", module_name)?;
    ...
    writeln!(file, "}}")?;
    writeln!(file)?;
    writeln!(file, "/// Returns the total number of registered tags across all families")?;
    writeln!(file, "pub fn generated_tag_count() -> usize {{")?;
    writeln!(file, "    {}", tags.len())?;
    writeln!(file, "}}")?;
    writeln!(file)?;
    writeln!(file, "/// Combined registry - lazily merged from all families on first access")?;
    writeln!(file, "/// WARNING: First access will merge 32K+ tags and may take a few hundred milliseconds")?;
    ...
    for (module_name, _) in &family_modules {
        writeln!(file, "    registry.extend({}::get_tags().iter().map(|(k, v)| (k.clone(), v.clone())));", module_name)?;
    }
    ...
    writeln!(file, "        assert!(generated_tag_count() >= {});", MIN_TAG_COUNT)?;
    writeln!(file, "    }}")?;
    writeln!(file, "}}")?;

    println!("cargo:warning=Generated {} family modules", family_modules.len());
    println!("cargo:warning=Generated main module at {}", GENERATED_TAGS_PATH);
    Ok(())
}
...
fn generate_family_module(path: &Path, family: &str, tags: &[&TagDefinition]) -> Result<()> {
    ...
    writeln!(file)?;
    writeln!(file, "use crate::tag_db::{{FormatFamily, TagDescriptor, TagId, ValueType}};")?;
    writeln!(file, "use once_cell::sync::Lazy;")?;
    writeln!(file, "use std::collections::HashMap;")?;
    writeln!(file)?;

    writeln!(file, "static TAGS: Lazy<Vec<TagDescriptor>> = Lazy::new(|| vec![")?;
    for tag in tags {
    ...
    writeln!(file, "pub fn get_tags() -> &'static HashMap<String, TagDescriptor> {{")?;
    writeln!(file, "    static MAP: Lazy<HashMap<String, TagDescriptor>> = Lazy::new(|| {{")?;
    writeln!(file, "        let mut map = HashMap::with_capacity(TAGS.len());")?;
    writeln!(file, "        for tag in TAGS.iter() {{")?;
    writeln!(file, "            map.insert(tag.tag_name.clone(), tag.clone());")?;
    writeln!(file, "        }}")?;
    writeln!(file, "        map")?;
    writeln!(file, "    }});")?;
    writeln!(file, "    &MAP")?;
