...
const EXIFTOOL_ARCHIVE_URL: &str = "https://github.com/exiftool/exiftool/archive/refs/heads/master.zip";
    ...
    match generate_tag_database() {
        Ok(tag_count) => {
            if tag_count >= MIN_TAG_COUNT {
                println!("cargo:warning=Successfully generated tag database with {} tags", tag_count);
            } else {
        ...
        Err(e) => {
            eprintln!("cargo:warning=Tag generation failed: {}. Using fallback to manual registry.", e);
            create_fallback_generated_tags().unwrap_or_else(|e| {
                panic!("Failed to create fallback tag database: {}", e);
            });
        }
...
fn generate_tag_database() -> Result<usize> {
    ...
    let source_dir = download_exiftool_source()
        .context("Failed to download ExifTool source")?;

    // Step 2: Parse tag definitions from Perl modules
    let tags = parse_exiftool_tags(&source_dir)
        .context("Failed to parse ExifTool tags")?;

    println!("cargo:warning=Parsed {} tags from ExifTool source", tags.len());

    // Step 3: Generate Rust code
    generate_rust_code(&tags)
    ...
    for (module_file, format_family) in modules {
        ...
        if module_path.exists() {
            match parse_perl_module(&module_path, format_family) {
                Ok(mut tags) => {
                    println!("cargo:warning=Parsed {} tags from {}", tags.len(), module_file);
...
fn create_tag_definition(
    ...
    let writable = data.get("Writable").map(|w| w != "no" && w != "0").unwrap_or(false);

    // Determine value type from Writable field
    let value_type = data.get("Writable")
...
fn map_perl_type_to_value_type(perl_type: &str) -> ValueType {
    match perl_type {
        ...
        "int8u" | "int8s" | "int16u" | "int16s" | "int32u" | "int32s" | "int64u" | "int64s" => ValueType::Integer,
...
fn generate_rust_code(tags: &[TagDefinition]) -> Result<()> {
    ...
    writeln!(file, "//!")?;
    writeln!(file, "//! Generated from ExifTool GitHub repository: {}", EXIFTOOL_REPO_URL)?;
    writeln!(file, "//! Total tags: {}", tags.len())?;
    writeln!(file)?;
    writeln!(file, "#![allow(dead_code)]")?;
    writeln!(file)?;
    writeln!(file, "use crate::core::tag_descriptor::{{FormatFamily, TagDescriptor, TagId, ValueType}};")?;
    writeln!(file, "use once_cell::sync::Lazy;")?;
    writeln!(file, "use std::collections::HashMap;")?;
    writeln!(file)?;
    writeln!(file, "/// Auto-generated tag registry from ExifTool source.")?;
    writeln!(file, "/// This registry is populated during the build process by parsing")?;
    writeln!(file, "/// ExifTool Perl modules and extracting tag metadata.")?;
    writeln!(file, "pub static GENERATED_TAG_REGISTRY: Lazy<HashMap<&'static str, TagDescriptor>> = Lazy::new(|| {{")?;
    writeln!(file, "    let mut registry = HashMap::with_capacity({});", tags.len())?;
    writeln!(file)?;
    ...
    writeln!(file, "/// * `name` - The full tag name (e.g., \"EXIF:Make\", \"XMP-dc:Creator\")")?;
    writeln!(file, "///")?;
    writeln!(file, "/// # Returns")?;
    writeln!(file, "/// * `Some(&TagDescriptor)` if the tag is registered")?;
    ...
    writeln!(file, "/// Returns the total number of registered tags in the generated registry.")?;
    ...
    writeln!(file, "        assert!(generated_tag_count() > 0, \"Generated registry should not be empty\");")?;
    writeln!(file, "    }}")?;
    writeln!(file)?;
    writeln!(file, "    #[test]")?;
    writeln!(file, "    fn test_generated_registry_min_count() {{")?;
    writeln!(file, "        assert!(generated_tag_count() >= {}, \"Expected at least {} tags\");", MIN_TAG_COUNT, MIN_TAG_COUNT)?;
    writeln!(file, "    }}")?;
    writeln!(file, "}}")?;

    println!("cargo:warning=Generated Rust code written to {}", GENERATED_TAGS_PATH);
    Ok(())
}
...
fn generate_tag_insertion(file: &mut File, tag: &TagDefinition) -> Result<()> {
    ...
    match &tag.tag_id {
        ...
        TagId::Named(s) => writeln!(file, "            TagId::new_named(\"{}\".to_string()),", escape_string(s))?,
    }

    // Tag name
    writeln!(file, "            \"{}\".to_string(),", escape_string(&tag.tag_name))?;
    ...
    // Description
    writeln!(file, "            \"{}\".to_string(),", escape_string(&tag.description))?;
...
fn create_fallback_generated_tags() -> Result<()> {
    ...
    writeln!(file, "//!")?;
    writeln!(file, "//! Tag generation from ExifTool source failed during build.")?;
    writeln!(file, "//! This file provides a fallback that delegates to the manually")?;
    writeln!(file, "//! curated tag registry to ensure the build succeeds.")?;
    writeln!(file)?;
    writeln!(file, "#![allow(dead_code)]")?;
    writeln!(file)?;
    writeln!(file, "use crate::core::tag_descriptor::TagDescriptor;")?;
    writeln!(file)?;
    writeln!(file, "/// Fallback lookup function (delegates to manual registry)")?;
    writeln!(file, "pub fn get_generated_tag_descriptor(name: &str) -> Option<&TagDescriptor> {{")?;
    writeln!(file, "    crate::tag_db::tag_registry::get_tag_descriptor(name)")?;
    writeln!(file, "}}")?;
    writeln!(file)?;
    writeln!(file, "/// Fallback tag count function (delegates to manual registry)")?;
    writeln!(file, "pub fn generated_tag_count() -> usize {{")?;
